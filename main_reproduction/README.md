...
| `table9_corpus_transition_sizes/` | Table 9 (corpus sizes 96–400) + Section 7.1 prose (Pearson/Spearman correlation) | `scratch_table9_transition_sizes_CLEAN.pkl` + `scratch_wilson_ci_transition_sizes.py`/`_results.pkl` + `scratch_correlation_transition_sizes.py`/`_results.pkl` | clean |
...
| `corpus_scarcity_finding/` | Section 7 prose (corpus-scarcity finding: fraction of pairs clearing a 0.5 fidelity threshold, 480- and 48-item corpora) | `scratch_verify_scarcity_oos.py`/`_results.pkl` | clean for PCA at both scales; the 48-item random figure is reported in the paper as 9/48 (18.8%), this script's own out-of-sample result — no PCA/random fraction for the 48-item corpus was ever published before this file, so there is nothing to reconcile with. An older, never-published "8/48" reference exists only as a hardcoded comment in a superseded script and is not reproducible from any surviving code; that history is kept here for transparency but is deliberately not carried into the manuscript (see folder README) |
