"""
scratch_verify_pca_leakage_minilm_noisesweep_nq8.py — full leak-free
correction for the MiniLM-embedded 48-item hard corpus noise sweep at
n_qubits=8 ("tab:noise-hard"), matching benchmark_query_noise_realdata_hard.py's
actual methodology exactly: n_seeds=5 (corpus rebuilt fresh per seed via
make_hard_real_corpus), n_queries=10/seed (n=50/cell total), all 10
multipliers. Recomputes BOTH leaky and leak-free fresh in the same run
(the archived .txt predates a pool-size change, so a fresh leaky baseline
is needed for a clean apples-to-apples comparison -- same practice as the
other 48-item corpus checks this investigation).
"""
import time
import numpy as np
import pickle
...
print(f"=== MiniLM 48-item hard corpus: full noise sweep, n_qubits=8, n_seeds=5 x n_queries=10 ===")
...
for mult in NOISE_MULTIPLIERS:
    ...
    # classical: raw-embedding cosine, same seeds/queries, independent of any projection fit
    classical_recalls = []
    for seed in range(N_SEEDS):
        corpus, keys = make_hard_real_corpus(48, 8, seed=seed)
        d_dim = next(iter(corpus.values())).shape[0]
...
print("\n--- MiniLM n_qubits=8 noise sweep: leaky vs leak-free, all 10 multipliers ---")
...
with open("scratch_pca_leakage_minilm_noisesweep_nq8_results.pkl", "wb") as f:
