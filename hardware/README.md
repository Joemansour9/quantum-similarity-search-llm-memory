...
│   ├── scratch_hw_gatecount_ttest.py / _results.pkl    (paper's paired n=4 CSWAP-vs-destructive
│   │                                 t-test, Section 8.2: reads the Priority 2 job's raw per-pair
│   │                                 fidelities and recomputes mean/std/paired t-test from scratch)
│   └── scratch_verify_multiquery_wilson.py / _results.pkl  (paper's Table tab:hw-multiquery Wilson
│                                     95% CI, Section 8.3: recomputes top-1 correctness per query
│                                     from the three Test5/Test12 result files and the CI from that)
